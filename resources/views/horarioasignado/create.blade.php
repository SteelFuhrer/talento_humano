...
<div class="container">
    <div class="card" style="margin: 10px; max-width: 1200px; margin: auto;">
        <h2 class="card-header">Crear Horario Asignado</h2>
        <div class="card-body">
            <form action="{{ route('horarioasignado.store') }}" method="POST">
                @csrf
                @if($errors->any())
                ...
                <div class="form-group">
                    ...
                    <input type="date" name="FAsignacion" class="form-control" value="{{ old('FAsignacion') }}" required>
                </div>
                <div class="form-group">
                    <label for="CausaHorario">Causa del Horario</label>
                    <input type="text" name="CausaHorario" class="form-control" value="{{ old('CausaHorario') }}" required>
                </div>
        </div>
        <div class="card-footer">
            <button type="submit" class="btn btn-primary"><i class="fa-solid fa-floppy-disk"></i> Crear</button>
            <button type="button" id="regresar" class="btn btn-secondary" onclick="window.location.href='{{ route('horarioasignado.index') }}'">
